...
from gamedef import (
    ClearGameDefCache,
    GAMEDEF_ONLINE_PATCH_ROOT,
    GAMEDEF_PATCH_ROOT,
    GameDefMenu,
    LoadGameDef,
    ReggieGameDefinition,
    ResolveGameDefDirectory,
    getAvailableGameDefs,
)
...
_qpt_functions = None
STARTUP_CLI_OPTIONS = {}
COLLAB_RUNTIME_INSTANCE_ID = uuid.uuid4().hex[:12]
...
def _get_collab_runtime_dir():
    """
    Return a per-process collaboration runtime directory.

    Multiple Reggie instances launched from the same folder must not share
    transient collaboration caches, otherwise localhost host/client sessions
    can race on downloaded tilesets and patch staging files.
    """
    path = os.path.join(_get_reggie_base_dir(), 'collab_runtime', COLLAB_RUNTIME_INSTANCE_ID)
    try:
...
def _get_collab_debug_log_path():
    return os.path.join(_get_collab_runtime_dir(), 'collaboration-debug.log')


def _collab_runtime_logs_enabled():
    try:
        return bool(setting('CollabRuntimeLogsEnabled', False))
    except Exception:
        return False
...
class CollaborationMonitorDialog(QtWidgets.QDialog):
    def __init__(self, participant_callback=None, ban_list_callback=None, parent=None):
        ...
        self.listWidget.customContextMenuRequested.connect(self._HandleContextMenuRequested)
        layout.addWidget(self.listWidget, 1)
    ...
    def setHostMode(self, is_host):
        ...
        if self._is_host:
            self.infoLabel.setText('Online participants. Left-click to follow, right-click to kick or ban.')
        else:
            self.infoLabel.setText('Online participants. Left-click to follow.')
    ...
    def setParticipants(self, participants):
        ...
        for participant in participants:
            ...
            level_name = str(participant.get('level_name') or '').strip()
            try:
                area_num = int(participant.get('area_num', 0) or 0)
            except Exception:
                area_num = 0
            ...
            if level_name:
                if area_num > 0:
                    tail = '  |  %s  |  Area %d' % (level_name, area_num)
                else:
                    tail = '  |  %s' % level_name

            nickname = str(participant.get('nickname') or 'Player')
            suffix = '  |  %s  |  %s%s' % (
            ...
            self.listWidget.addItem(item)
            row = QtWidgets.QWidget(self.listWidget)
            ...
            nick_label = QtWidgets.QLabel(nickname, row)
            nick_label.setAttribute(QtCore.Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
            nick_label.setStyleSheet(
                'background: transparent; color: %s; font-weight: bold;' % (
                    normalize_collab_color(participant.get('highlight_color')),
                )
            )

            suffix_label = QtWidgets.QLabel(suffix, row)
            suffix_label.setAttribute(QtCore.Qt.WidgetAttribute.WA_TransparentForMouseEvents, True)
            suffix_label.setStyleSheet('background: transparent; color: white;')

            row_layout.addWidget(nick_label, 0)
            row_layout.addWidget(suffix_label, 1)
    ...
    def _HandleItemClicked(self, item):
        ...
        if callable(self._participant_callback):
            self._participant_callback(dict(participant or {}), None, QtCore.Qt.MouseButton.LeftButton)

    def _HandleContextMenuRequested(self, pos):
        ...
        if callable(self._participant_callback):
            ...
            self._participant_callback(dict(participant or {}), global_pos, QtCore.Qt.MouseButton.RightButton)
...
class ReggieWindow(QtWidgets.QMainWindow):
    ...
    def __init2__(self):
        ...
        self.collabManager.set_debug_logger(self._HandleCollabManagerDebugLog)
...
#       self.collabManager.set_peer_intro_validator(self._ValidateCollabPeerIntro)
        ...
        self.collabCursorDisplayMode = self._NormalizeCollabCursorDisplayMode(getattr(globals_, 'CollabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN))
        ...
        self._collabFocusCursorSid = None
        self._collabFocusCursorDueAt = 0.0
        self._collabFocusCursorTimer = QtCore.QTimer(self)
        ...
        self._collabTilesetSyncWatchdogTimer = QtCore.QTimer(self)
        self._collabTilesetSyncWatchdogTimer.setSingleShot(True)
        self._collabTilesetSyncWatchdogTimer.timeout.connect(self._HandleCollabTilesetSyncWatchdogTimeout)
    ...
    def _CollabDebugLog(self, event, **fields):
        if not _collab_runtime_logs_enabled():
            return
        timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
        parts = [
            timestamp,
            'pid=%s' % os.getpid(),
            'instance=%s' % COLLAB_RUNTIME_INSTANCE_ID,
            'event=%s' % str(event or ''),
        ]
        for key in sorted(fields):
            ...
            try:
                ...
                rendered = '<unreprable>'
            parts.append('%s=%s' % (str(key), rendered.replace('\r', '\\r').replace('\n', '\\n')))
        try:
            with open(_get_collab_debug_log_path(), 'a', encoding='utf-8') as f:
                f.write(' | '.join(parts) + '\n')
        except Exception:
            pass

    def _HandleCollabManagerDebugLog(self, **fields):
        event = str(fields.pop('event', '') or '')
        self._CollabDebugLog('manager.%s' % event, **fields)
    ...
    def createMenubar(self):
        ...
        self.CreateAction(
            'collabcursor_on_screen', lambda checked=False: self.HandleCollabCursorDisplayModeChanged(COLLAB_CURSOR_DISPLAY_ON_SCREEN, checked), None,
            'When on screen', 'Display collaboration cursors only while they are on-screen, but keep ping on P enabled',
            None, True,
        )
        ...
        collab_cursor_menu.addAction(self.actions['collabcursor_on_screen'])
        ...
        try:
            ...
            self.collabMonitorButton.setFixedHeight(button_height)
            self.collabChatButton.setFixedHeight(button_height)
        except Exception:
            pass
    ...
    def _CollabPeerNavigationStateInfo(self, session_id, participant=None):
        sid = str(session_id or '')
        if not sid:
            return {}

        if sid == self._CollabLocalSessionId():
            return self._CollabCurrentEditorStateInfo()
        ...
        level_name = str(state.get('level_name') or '').strip() if isinstance(state, dict) else ''
        try:
            area_num = int(state.get('area_num', 0) or 0) if isinstance(state, dict) else 0
        except Exception:
            area_num = 0
        if level_name or area_num > 0:
            return {
        ...
        cursor = dict((getattr(self, 'collabRemoteCursors', {}) or {}).get(sid) or {})
        if not cursor:
            return {}
        try:
            ...
            area_num = 0
        return {
            'level_name': str(cursor.get('level_name') or '').strip(),
            'area_num': area_num,
        }
    ...
    def _NormalizeCollabCursorDisplayMode(self, mode):
        ...
        if mode not in COLLAB_CURSOR_DISPLAY_MODES:
            return COLLAB_CURSOR_DISPLAY_ON_SCREEN
        return mode

    def _UpdateCollabCursorDisplayActions(self):
        mode = self._NormalizeCollabCursorDisplayMode(getattr(self, 'collabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN))
        mapping = (
            ('collabcursor_always', COLLAB_CURSOR_DISPLAY_ALWAYS),
            ('collabcursor_on_screen', COLLAB_CURSOR_DISPLAY_ON_SCREEN),
    ...
    def _ShouldDisplayCollabCursors(self):
        ...
        if mode == COLLAB_CURSOR_DISPLAY_ON_P:
            return bool(getattr(self, 'collabPointerKeyHeld', False))
        return True

    def HandleCollabCursorDisplayModeChanged(self, mode, checked=False):
        mode = self._NormalizeCollabCursorDisplayMode(mode)
        if not checked and mode == getattr(self, 'collabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN):
            self._UpdateCollabCursorDisplayActions()
            return
    ...
    def _BuildCollabMonitorParticipants(self):
        participants = []
        local_sid = self._CollabLocalSessionId()
        raw = list(getattr(self, 'collabParticipants', []) or [])
        if local_sid and all(str(p.get('session_id') or '') != local_sid for p in raw if isinstance(p, dict)):
            raw.insert(0, {
                'session_id': local_sid,
                'nickname': getattr(self, 'collabSelfNick', 'Player'),
                'highlight_color': getattr(self, 'collabSelfHighlightColor', DEFAULT_COLLAB_HIGHLIGHT_COLOR),
        ...
        for participant in raw:
            ...
            if sid:
                state = self._CollabPeerNavigationStateInfo(sid, participant=entry)
                if isinstance(state, dict):
                    entry['level_name'] = state.get('level_name')
                    entry['area_num'] = state.get('area_num')
                if not entry.get('nickname'):
                    entry['nickname'] = self._CollabPeerDisplayName(sid)
                entry['highlight_color'] = normalize_collab_color(
                    entry.get('highlight_color') or self._CollabPeerColor(sid)
                )
            participants.append(entry)
        return participants

    def _RefreshCollabMonitorDialog(self):
        if not hasattr(self, 'collabMonitorDialog') or self.collabMonitorDialog is None:
            return
        self.collabMonitorDialog.setHostMode(hasattr(self, 'collabManager') and self.collabManager.mode == "host")
        self.collabMonitorDialog.setParticipants(self._BuildCollabMonitorParticipants())
    ...
    def HandleCollabPingShortcut(self):
        if self._NormalizeCollabCursorDisplayMode(getattr(self, 'collabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN)) == COLLAB_CURSOR_DISPLAY_NEVER:
            return
        scene_pos = self._CurrentCollabPingScenePos()
        if scene_pos is None:
            return
    ...
    def DrawCollabRemoteCursors(self, view, painter):
        ...
        mode = self._NormalizeCollabCursorDisplayMode(getattr(self, 'collabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN))
        force_edge = bool(getattr(self, 'collabPointerKeyHeld', False))
        viewport_rect = view.viewport().rect()
        font = painter.font()
        font.setBold(True)
        margin = 18.0
        ...
        for sid, cursor in list(self.collabRemoteCursors.items()):
            ...
            if mode == COLLAB_CURSOR_DISPLAY_ON_SCREEN and not force_edge:
                if not viewport_rect.contains(QtCore.QPoint(int(target.x()), int(target.y()))):
                    continue
    ...
    def _HandleCollabMonitorParticipant(self, participant, global_pos=None, mouse_button=None):
        if not isinstance(participant, dict):
            return
        if mouse_button == QtCore.Qt.MouseButton.LeftButton:
            self._CollabFollowParticipant(participant)
            return
    ...
    def _CollabFollowParticipant(self, participant):
        if not isinstance(participant, dict):
            return
        session_id = str(participant.get('session_id') or '')
        if not session_id:
            return
        state = self._CollabPeerNavigationStateInfo(session_id, participant=participant)
        ...
        same_level = False
        try:
            cur_level = str(self._CollabCurrentLevelName() or '')
            cur_area = int(getattr(globals_.Area, 'areanum', 0) or 0) if globals_.Area is not None else 0
            same_level = (self._CollabNormalizeLevelName(cur_level) == self._CollabNormalizeLevelName(level_name) and cur_area == area_num)
        except Exception:
            same_level = False
        ...
        if not same_level:
            if self.IsCollabClientMode():
                try:
                    self._CollabRequestHostedLevelOpen(level_name, area_num)
                except Exception:
                    pass
            else:
                try:
                    ...
                    pass
                try:
                    self.LoadLevel(level_name, False, area_num)
                except Exception:
                    pass

        self._RequestCollabFocusCursor(session_id)

    def _RequestCollabFocusCursor(self, session_id):
        session_id = str(session_id or '')
        if not session_id:
            return
        self._collabFocusCursorSid = session_id
        self._collabFocusCursorDueAt = time.monotonic() + 8.0
        try:
            self._collabFocusCursorTimer.start(0)
        except Exception:
            pass

    def _TryFocusCollabCursor(self, session_id):
        session_id = str(session_id or '')
        if not session_id:
            return False
        if not hasattr(self, 'view') or self.view is None:
        ...
        local_sid = self._CollabLocalSessionId()
        scene_pos = None
        if session_id == local_sid:
            scene_pos = getattr(self, 'collabLastMouseScenePos', None)
        else:
            cursor = getattr(self, 'collabRemoteCursors', {}).get(session_id)
            ...
            try:
                scene_pos = QtCore.QPointF(
                    float(cursor.get('target_x', 0.0) or 0.0),
                    float(cursor.get('target_y', 0.0) or 0.0),
                )
            except Exception:
                scene_pos = None
        if scene_pos is None:
    ...
    def _FlushCollabFocusCursor(self):
        session_id = str(getattr(self, '_collabFocusCursorSid', '') or '')
        if not session_id:
            return
        if time.monotonic() > float(getattr(self, '_collabFocusCursorDueAt', 0.0) or 0.0):
            self._collabFocusCursorSid = None
            return
        if self._TryFocusCollabCursor(session_id):
            self._collabFocusCursorSid = None
            return
        try:
            self._collabFocusCursorTimer.start(80)
        except Exception:
            pass
    ...
    def SetupDocksAndPanels(self):
        ...
        self.spritePreviewButton.setAutoRaise(False)
        self.spritePreviewButton.setStyleSheet(
            'QToolButton { background-color: palette(button); color: palette(button-text); border: 1px solid palette(mid); border-radius: 6px; padding: 3px 6px; }'
            'QToolButton:hover { border: 2px solid palette(highlight); }'
            'QToolButton:pressed { background-color: palette(dark); }'
            'QToolButton:checked { background-color: palette(button); color: palette(button-text); border: 1px solid palette(mid); }'
        )
    ...
    def _GetTilesetCacheDir(self):
        path = os.path.join(_get_collab_runtime_dir(), 'collab_tilesets')
    ...
    def HandleTilesetEditClicked(self, checked=False):
        ...
        initial_signature = self._GetTilesetEditFileSignature(override_path)

        # (Re)start file watcher for this tileset edit session.
        self._tilesetEditSession = {
            'name': name,
            'path': override_path,
            'flush_retry_count': 0,
            'last_seen_signature': initial_signature,
            'stable_signature_candidate': None,
        }
        try:
    ...
    def _GetTilesetEditFileSignature(self, path):
        path = str(path or '')
        if not path:
            return None
        try:
    ...
    def _PollTilesetEditorProcess(self):
        ...
        try:
            ...
            current_signature = self._GetTilesetEditFileSignature(path)
            previous_signature = session.get('last_seen_signature')
            if current_signature is not None and current_signature != previous_signature:
                session['last_seen_signature'] = current_signature
                self._tilesetEditPendingPath = path
                self._tilesetEditDebounce.start(150)
        except Exception:
            pass
    ...
    def _FlushTilesetEditorFileChanged(self):
        ...
        current_signature = self._GetTilesetEditFileSignature(path)
        stable_signature_candidate = self._tilesetEditSession.get('stable_signature_candidate')
        if current_signature is None:
            if self._RetryTilesetEditorFileChanged(150):
                return
            return
        if stable_signature_candidate != current_signature:
            self._tilesetEditSession['stable_signature_candidate'] = current_signature
            if self._RetryTilesetEditorFileChanged(150):
                return
        ...
        try:
            ...
            return
        try:
            self._tilesetEditSession['last_seen_signature'] = current_signature
            self._tilesetEditSession['stable_signature_candidate'] = None
        except Exception:
            pass
    ...
    def _ReloadTilesetNamesEverywhere(self, names):
        ordered_names = []
        seen_names = set()
        ...
        if not ordered_names or globals_.Area is None:
            return
        try:
        ...
        for name in ordered_names:
            for slot in self._GetTilesetSlotsForName(name):
                try:
                    slot = int(slot)
                except Exception:
                    continue
        ...
        for slot in slots:
            try:
                UnloadTileset(slot)
                LoadTileset(slot, self._GetTilesetNameForSlot(slot), reload_=True)
            except Exception:
                pass
    ...
    def _BuildCollabKnownTilesetHashes(self, include_all_game_tilesets=False):
        ...
        if include_all_game_tilesets:
            ...
            try:
                ...
                final_dir = None
            allow_final_dir = bool(final_dir)
            if allow_final_dir and self.IsCollabClientMode():
                current_root = self._CurrentCollabGameRootPath()
                # Same-folder localhost sessions can share Stage\Texture.
                # Only trust the final dir as a private cache when we are using
                # a downloaded online patch root.
                allow_final_dir = (current_root == GAMEDEF_ONLINE_PATCH_ROOT)
                if not allow_final_dir:
                    self._CollabDebugLog(
                        'tileset.known_hashes.skip_final_dir',
                        final_dir=final_dir,
                        reason='shared_stage_texture_directory',
                    )
            if final_dir and allow_final_dir:
    ...
    def _TouchCollabTilesetSyncProgress(self, reason):
        state = getattr(self, '_collabTilesetSyncState', None)
        if not isinstance(state, dict):
            try:
                self._collabTilesetSyncWatchdogTimer.stop()
            except Exception:
                pass
            return

        state['last_progress_at'] = time.monotonic()
        state['last_progress_reason'] = str(reason or '')
        try:
            self._collabTilesetSyncWatchdogTimer.start(15000)
        except Exception:
            pass

    def _HandleCollabTilesetSyncWatchdogTimeout(self):
        state = getattr(self, '_collabTilesetSyncState', None)
        if not isinstance(state, dict):
            return
        ...
        pending_payloads = len(getattr(self, '_collabPendingTilesetPayloads', []) or [])
        queue_ = state.get('apply_queue')
        queue_len = len(queue_) if isinstance(queue_, collections.deque) else 0
        self._CollabDebugLog(
            'tileset.sync.watchdog_timeout',
            phase=phase,
            received_files=received_files,
            total_files=total_files,
            host_done_seen=bool(state.get('host_done_seen')),
            pending_payloads=pending_payloads,
            apply_queue_len=queue_len,
            last_progress_reason=str(state.get('last_progress_reason') or ''),
            expected_files=sorted(str(name or '') for name in expected_files.keys() if str(name or ''))[:24],
        )

        if phase == 'apply':
            if queue_len or pending_payloads:
                self._TouchCollabTilesetSyncProgress('watchdog.resume_apply')
                try:
        ...
        if total_files <= 0 and not expected_files:
        ...
        retry_count = int(state.get('watchdog_retries', 0) or 0)
        if retry_count < 1:
            state['watchdog_retries'] = retry_count + 1
            self._CollabDebugLog(
                'tileset.sync.watchdog_retry',
                retry_count=state['watchdog_retries'],
                pending_payloads=pending_payloads,
                remaining_expected=sorted(str(name or '') for name in expected_files.keys() if str(name or ''))[:24],
            )
            self._TouchCollabTilesetSyncProgress('sync.watchdog_retry')
            try:
                sent = self.collabManager.broadcast_message('tileset_sync_request', {
                    ...
                    'include_all_game_tilesets': bool(state.get('include_all_game_tilesets')),
                    'known_files': self._BuildCollabKnownTilesetHashes(
                        include_all_game_tilesets=bool(state.get('include_all_game_tilesets'))
                    ),
                })
            except Exception:
        ...
        missing_names = sorted(str(name or '') for name in expected_files.keys() if str(name or ''))
        self._FailCollabTilesetSync(
            'Timed out waiting for host tilesets.\nMissing tilesets: %s'
            % (', '.join(missing_names[:8]) if missing_names else 'unknown'),
            disconnect=True,
        )
    ...
    def _StartCollabTilesetSyncProgress(self):
        ...
        self._collabTilesetSyncState = {
            ...
            'apply_total': 0,
            'applied_files': 0,
            'apply_changed_names': set(),
            'host_done_seen': False,
            'host_done_retries': 0,
            'watchdog_retries': 0,
        }
        self._CollabDebugLog('tileset.sync.start')
        self._RefreshCollabInteractionBlock()
        self._UpdateCollabTilesetProgress('Requesting patch tilesets...')
        self._TouchCollabTilesetSyncProgress('sync.start')

    def _FinishCollabTilesetSyncWithoutDownloads(self):
        self._CollabDebugLog('tileset.sync.finish_without_downloads')
        self._ResetCollabTilesetSyncState()
        try:
    ...
    def _MaybeContinueDeferredCollabTilesetSync(self):
        state = getattr(self, '_collabTilesetSyncState', None)
        if not isinstance(state, dict):
            return False
        if not bool(state.get('host_done_seen')):
            return False
        if globals_.Area is None or globals_.Level is None:
            return False
        total_files = int(state.get('total_files', 0) or 0)
        ...
        if total_files > 0 and received_files < total_files:
            return False
        self._CollabDebugLog(
            'tileset.sync.resume_after_level_ready',
            received_files=received_files,
            total_files=total_files,
            pending_payloads=len(getattr(self, '_collabPendingTilesetPayloads', []) or []),
        )
    ...
    def _PrimeCollabTilesetSyncFromLocalFiles(self):
        ...
        self._CollabDebugLog(
            'tileset.sync.prime_local_matches',
            matched_count=len(matched_names),
            matched_names=matched_names,
            matched_bytes=matched_bytes,
            remaining_expected=sorted(expected_files.keys()),
        )
        self._TouchCollabTilesetSyncProgress('sync.prime_local_matches')
    ...
    def _TrackReceivedCollabTileset(self, name, data):
        ...
        self._CollabDebugLog(
            'tileset.sync.received_file',
            name=name,
            received_files=state.get('received_files', 0),
            total_files=state.get('total_files', 0),
            expected_size=expected.get('size', 0),
            payload_size=len(data or b''),
        )
        self._TouchCollabTilesetSyncProgress('sync.received_file:%s' % name)
    ...
    def _BeginDeferredCollabTilesetApply(self):
        ...
        self._UpdateCollabTilesetProgress()
        self._TouchCollabTilesetSyncProgress('sync.begin_apply')
        try:
            self._collabTilesetApplyTimer.start(0)
        except Exception:
            self._ProcessPendingTilesetPayloadBatch()
    ...
    def _FinalizeCollabTilesetSyncFromHostDone(self, payload=None):
        ...
        self._CollabDebugLog(
            'tileset.sync.host_done',
            payload=payload or {},
            received_files=state.get('received_files', 0),
            total_files=state.get('total_files', 0),
            expected_files=sorted((state.get('expected_files') or {}).keys()),
        )
        self._TouchCollabTilesetSyncProgress('sync.host_done')
        ...
        if globals_.Area is None or globals_.Level is None:
            self._CollabDebugLog(
                'tileset.sync.host_done.defer_until_level_ready',
                received_files=received_files,
                total_files=total_files,
            )
            return
    ...
    def _CompleteDeferredCollabTilesetApply(self):
        self._CollabDebugLog('tileset.sync.apply_complete')
    ...
    def _ProcessPendingTilesetPayloadBatch(self):
        ...
        processed_any = False
        changed_names = state.get('apply_changed_names')
        if not isinstance(changed_names, set):
            changed_names = set()
            state['apply_changed_names'] = changed_names
        ...
        while queue_:
            ...
            if name and data:
                self._CollabDebugLog(
                    'tileset.sync.apply_file',
                    name=name,
                    slots=slots or [],
                    queue_remaining=len(queue_),
                )
                if self._ApplyCollabTilesetBytes(
                    name,
                    data,
                    slots=slots,
                    broadcast=False,
                    ...
                    changed_names.add(str(name))
                state['applied_files'] = int(state.get('applied_files', 0) or 0) + 1
                self._TouchCollabTilesetSyncProgress('sync.apply_file:%s' % name)
                processed_any = True
        ...
        if changed_names:
            self._ReloadTilesetNamesEverywhere(changed_names)

        self._CompleteDeferredCollabTilesetApply()

    def _ApplyCollabTilesetBytes(self, name, data, slots=None, broadcast=False, target_peer=None, reload_ui=True):
        """
        ...
        if raw_sha1 and raw_sha1 == str(self._collabTilesetSha1ByName.get(name) or ''):
            ...
            self._CollabDebugLog('tileset.apply.skip_same_sha1', name=name, sha1=raw_sha1)
            return False
        ...
        try:
            ...
            self._CollabDebugLog('tileset.apply.write_failed', name=name, override_path=override_path)
            return False
        ...
        if reload_ui:
            self._ReloadTilesetNameEverywhere(name)
        self._CollabDebugLog('tileset.apply.done', name=name, override_path=override_path)
    ...
    def _RequestHostTilesetsNow(self, include_all_game_tilesets=False, force=False):
        ...
        self._CollabDebugLog(
            'tileset.request_host_now',
            include_all_game_tilesets=include_all_game_tilesets,
            force=force,
            area_num=int(getattr(globals_.Area, 'areanum', 0) or 0) if globals_.Area is not None else 0,
            known_files_count=len(known_files),
            known_names=sorted(known_files.keys())[:24],
        )
        self._StartCollabTilesetSyncProgress()
        if isinstance(getattr(self, '_collabTilesetSyncState', None), dict):
            self._collabTilesetSyncState['include_all_game_tilesets'] = bool(include_all_game_tilesets)
        try:
            sent = self.collabManager.broadcast_message('tileset_sync_request', {
                'area_num': int(getattr(globals_.Area, 'areanum', 0) or 0) if globals_.Area is not None else None,
                'level_name': self._CollabCurrentLevelName() if globals_.Area is not None else '',
        ...
        if not sent:
            self._CollabDebugLog(
                'tileset.request_host_now.failed_send',
                include_all_game_tilesets=include_all_game_tilesets,
                force=force,
            )
            self._ResetCollabTilesetSyncState()
    ...
    def _HostSendTilesetsToPeer(self, peer_session_id, area_num=None, include_all_game_tilesets=False, known_files=None):
        ...
        self._CollabDebugLog(
            'tileset.host_manifest',
            peer_session_id=peer_session_id,
            include_all_game_tilesets=include_all_game_tilesets,
            requested_area_num=area_num,
            entry_count=len(entries),
            download_count=len(download_entries),
            known_files_count=len(known_files),
            entry_names=[str(entry.get('name') or '') for entry in entries[:24]],
            download_names=[str(entry.get('name') or '') for entry in download_entries[:24]],
        )
    ...
    def _ApplyPendingTilesetPayloads(self):
        ...
        for _name, (payload, _sender, _data, _slots) in latest_by_name.items():
            name, data, slots = self._DecodeTilesetPayload(payload)
            if name and data:
                self._CollabDebugLog('tileset.apply_pending_payload', name=name, slots=slots or [])
                self._ApplyCollabTilesetBytes(name, data, slots=slots, broadcast=False)
    ...
    def _CurrentCollabGameRootPath(self):
        try:
            if getattr(globals_.gamedef, 'custom', False):
                return str(getattr(globals_.gamedef, 'patch_root_path', '') or '')
        except Exception:
            pass
        return ''

    def _FormatCollabBytes(self, value):
        try:
    ...
    def _CollectCollabPatchDefs(self, game_def_id=None, primary_root=None):
        normalized_id = self._NormalizeCollabGameDefId(game_def_id)
        if not normalized_id:
            return []

        prefer_online = (primary_root == GAMEDEF_ONLINE_PATCH_ROOT)
        try:
            root_def = ReggieGameDefinition(normalized_id, prefer_online=prefer_online)
            root_def.__init2__()
        except Exception:
            return []

        if primary_root:
            actual_root = str(getattr(root_def, 'patch_root_path', '') or '')
        ...
        def visit(def_):
            if not getattr(def_, 'custom', False):
                return
            key = (
                str(getattr(def_, 'patch_root_path', '') or ''),
                str(getattr(def_, 'gamepath', '') or ''),
            )
            if key in seen:
                return
            seen.add(key)
            ...
            imported_defs = list(getattr(def_, 'imported_defs', []) or [])
            imported_defs.sort(key=lambda item: (
                str(getattr(item, 'gamepath', '') or ''),
                str(getattr(item, 'patch_root_path', '') or ''),
            ))
            for imported in imported_defs:
                visit(imported)
            visit(getattr(def_, 'base', None))

        visit(root_def)
        return results

    def _EnumerateCollabPatchEntries(self, game_def_id=None, primary_root=None):
        entries = []
        for def_ in self._CollectCollabPatchDefs(game_def_id, primary_root=primary_root):
            patch_dir = os.path.join(str(getattr(def_, 'patch_root_path', '') or ''), str(getattr(def_, 'gamepath', '') or ''))
            if not os.path.isdir(patch_dir):
                continue

            patch_folder = str(getattr(def_, 'gamepath', '') or '')
            for root, dirs, files in os.walk(patch_dir):
                ...
                for filename in sorted(files):
                    ...
                    entries.append({
                        ...
                        'target_path': '%s/%s' % (patch_folder, rel_path),
                        'file_path': file_path,
                        'size': size,
                    })
    ...
    def _GetCollabGamePluginSummary(self, game_def_id=None, primary_root=None):
        normalized_id = self._NormalizeCollabGameDefId(game_def_id)
        if not normalized_id:
            return {'hash': 'builtin-nsmbw', 'file_count': 0, 'total_bytes': 0}

        if primary_root is None and normalized_id == self._CurrentCollabGameDefId():
            current_root = self._CurrentCollabGameRootPath()
            if current_root:
                primary_root = current_root

        cache = getattr(self, '_collabGamePluginSummaryCache', None)
        if cache is None:
            self._collabGamePluginSummaryCache = {}
            cache = self._collabGamePluginSummaryCache
        cache_key = (normalized_id, str(primary_root or ''))
        if cache_key in cache:
            return dict(cache[cache_key])

        entries = self._EnumerateCollabPatchEntries(normalized_id, primary_root=primary_root)
        if not entries:
            summary = {'hash': '', 'file_count': 0, 'total_bytes': 0}
            cache[cache_key] = dict(summary)
            return summary

        digest = hashlib.sha256()
        digest.update(b'reggie-collab-game-plugin-v2\0')
        total_bytes = 0

        for entry in entries:
            digest.update(entry['target_path'].encode('utf-8', 'surrogatepass'))
            digest.update(b'\0')
            try:
                ...
                return {'hash': '', 'file_count': 0, 'total_bytes': 0}
            digest.update(b'\0')
    ...
    def _GetCollabGamePluginHash(self, game_def_id=None, primary_root=None):
        return str(self._GetCollabGamePluginSummary(game_def_id, primary_root=primary_root).get('hash') or '')

    def _InvalidateCollabGamePluginCache(self, game_def_id=None):
        normalized_id = self._NormalizeCollabGameDefId(game_def_id)
        cache = getattr(self, '_collabGamePluginSummaryCache', None)
    ...
    def _CloseCollabPatchProgressDialog(self):
        dlg = getattr(self, '_collabPatchProgressDialog', None)
        if dlg is None:
            return
        try:
            dlg.close()
    ...
    def _UpdateCollabPatchProgress(self, label=None):
        state = getattr(self, '_collabPatchSyncState', None)
        if not isinstance(state, dict):
            return
        ...
        if dlg is None:
            ...
            btn.setEnabled(False)
            dlg.setCancelButton(btn)
            self._collabPatchProgressDialog = dlg
    ...
    def _ResetCollabPatchSyncState(self, cleanup_staging=True):
        state = getattr(self, '_collabPatchSyncState', None)
        self._collabPatchSyncState = None
        self._CloseCollabPatchProgressDialog()
        if cleanup_staging and isinstance(state, dict):
            staging_root = str(state.get('staging_root') or '')
            if staging_root:
                shutil.rmtree(staging_root, ignore_errors=True)

    def _FailCollabPatchSync(self, message, disconnect=False):
        self._ResetCollabPatchSyncState(cleanup_staging=True)
        QtWidgets.QMessageBox.warning(self, 'Collaboration', str(message))
        if disconnect and self._CollabEnabled():
            self.HandleCollabStop()

    def _StartCollabPatchSync(self, host_info):
        if not self.IsCollabClientMode():
            return False
        ...
        game_id = self._NormalizeCollabGameDefId(host_info.get('game_id'))
        host_name = str(host_info.get('game_name') or game_id or 'host game')
        host_hash = str(host_info.get('game_plugin_hash') or '')
        if not bool(host_info.get('game_is_custom')) or not game_id:
            return False

        online_hash = self._GetCollabGamePluginHash(game_id, primary_root=GAMEDEF_ONLINE_PATCH_ROOT)
        if host_hash and online_hash == host_hash:
            if (
                self._CurrentCollabGameDefId() != game_id
                or self._CurrentCollabGameRootPath() != GAMEDEF_ONLINE_PATCH_ROOT
            ):
                if not self._LoadCollabGameDef(game_id, prefer_online=True):
                    self._FailCollabPatchSync(
                        'Unable to load the cached online copy of the host game.\nHost game: %s' % host_name,
                        disconnect=True,
                    )
                    return True
            return False

        local_hash = self._GetCollabGamePluginHash(game_id, primary_root=GAMEDEF_PATCH_ROOT)
        if host_hash and local_hash == host_hash:
            if (
                self._CurrentCollabGameDefId() != game_id
                or self._CurrentCollabGameRootPath() != GAMEDEF_PATCH_ROOT
            ):
                if not self._LoadCollabGameDef(game_id):
                    self._FailCollabPatchSync(
                        'Unable to load the local copy of the host game.\nHost game: %s' % host_name,
                        disconnect=True,
                    )
                    return True
            return False

        online_root = GAMEDEF_ONLINE_PATCH_ROOT
        staging_root = os.path.join(_get_collab_runtime_dir(), 'patch_staging', game_id)
        os.makedirs(online_root, exist_ok=True)
        shutil.rmtree(staging_root, ignore_errors=True)
        os.makedirs(staging_root, exist_ok=True)

        self._collabPatchSyncState = {
            'game_id': game_id,
            'host_info': host_info,
    ...
    def _FinalizeCollabPatchSync(self, payload):
        state = getattr(self, '_collabPatchSyncState', None)
        if not isinstance(state, dict):
            return

        expected_files = state.get('expected_files') or {}
        received_paths = state.get('received_paths') or set()
        missing = [path for path in expected_files if path not in received_paths]
        if missing:
            self._FailCollabPatchSync(
                'The host patch download finished before all files were received.\nMissing files: %d' % len(missing),
                disconnect=True,
            )
            return
        ...
        expected_hash = str(payload.get('plugin_hash') or host_info.get('game_plugin_hash') or '')
        actual_hash = self._GetCollabGamePluginHash(game_id, primary_root=GAMEDEF_ONLINE_PATCH_ROOT)
        if expected_hash and actual_hash != expected_hash:
            self._FailCollabPatchSync(
                'The downloaded host patch checksum does not match the host.\nExpected: %s\nReceived: %s' % (
                    expected_hash,
    ...
    def _HostSendPatchFilesToPeer(self, peer_session_id, game_def_id=None, known_files=None):
        ...
        game_id = self._NormalizeCollabGameDefId(game_def_id)
        if not game_id:
            return
        ...
        for entry in entries:
            ...
            manifest_entries.append({
                ...
                'patch_folder': entry['patch_folder'],
                'size': int(entry['size']),
                'sha1': self._TilesetBytesSha1(raw),
            })
        ...
        for entry, raw in download_entries:
            ...
            try:
                if not self.collabManager.send_message_to(peer_session_id, 'patch_file_data', payload):
                    return
            except Exception:
                return
    ...
    def _BuildCollabRoomInfo(self):
        ...
        return {
            'game_id': game_id,
            'game_name': game_name,
            'game_plugin_hash': summary.get('hash', ''),
...
#               return 'Connection rejected. Plugin checksum mismatch for game "%s".' % str(host_info.get('game_name') or 'Unknown game')
    ...
    def _LoadCollabGameDef(self, game_def_id, prefer_online=False):
        ...
        try:
            ...
            result = LoadGameDef(game_def_id, dlg, prompt_for_stage_path=False, prefer_online=prefer_online)
        finally:
    ...
    def _EnsureCollabGameMatchesHost(self, host_info):
        ...
        if host_is_custom and host_game_id:
            online_hash = self._GetCollabGamePluginHash(host_game_id, primary_root=GAMEDEF_ONLINE_PATCH_ROOT)
            if host_plugin_hash and online_hash == host_plugin_hash:
                if (
                    self._CurrentCollabGameDefId() != host_game_id
                    or self._CurrentCollabGameRootPath() != GAMEDEF_ONLINE_PATCH_ROOT
                ):
                    if not self._LoadCollabGameDef(host_game_id, prefer_online=True):
                        QtWidgets.QMessageBox.warning(
                            self,
                            'Collaboration',
            ...
            local_hash = self._GetCollabGamePluginHash(host_game_id, primary_root=GAMEDEF_PATCH_ROOT)
            if host_plugin_hash and local_hash == host_plugin_hash:
                if (
                    self._CurrentCollabGameDefId() != host_game_id
                    or self._CurrentCollabGameRootPath() != GAMEDEF_PATCH_ROOT
                ):
                    if not self._LoadCollabGameDef(host_game_id):
                        QtWidgets.QMessageBox.warning(
                            self,
                            'Collaboration',
            ...
            local_exists = ResolveGameDefDirectory(host_game_id, primary_root=GAMEDEF_PATCH_ROOT) is not None
            online_exists = ResolveGameDefDirectory(host_game_id, primary_root=GAMEDEF_ONLINE_PATCH_ROOT) is not None
            if host_plugin_hash or (not local_exists and not online_exists):
                return True
...
#               return False
    ...
    def _CollabHostedStageFilePath(self, level_name):
        ...
        candidate = os.path.join(stage_dir, *level_name.split('/'))
        return self._ResolveExistingLevelPath(candidate) or candidate

    def _ResolveExistingLevelPath(self, path):
        path = str(path or '').strip()
        if not path:
            return ''
        ...
        match = re.search(r'(?i)\.arc(?:\.(?:lz|lh))?$', path)
        if match:
            stem = path[:match.start()]
            add_candidate(stem + '.arc')
            add_candidate(stem + '.arc.LZ')
            add_candidate(stem + '.arc.LH')
        else:
            for ext in getattr(globals_, 'FileExtentions', ('.arc', '.arc.LH', '.arc.LZ')) or ('.arc', '.arc.LH', '.arc.LZ'):
                add_candidate(path + ext)
    ...
    def HandleRemoteMessage(self, message, sender):
        ...
        if msg_type == 'patch_manifest':
            ...
            state['total_files'] = int(payload.get('file_count', len(expected_files)) or len(expected_files))
        ...
        if msg_type == 'patch_file_data':
            ...
            if not isinstance(state, dict):
                return
            target_path = str(payload.get('path') or '').strip().replace('\\', '/')
            expected = (state.get('expected_files') or {}).get(target_path)
            if not expected or target_path in (state.get('received_paths') or set()):
                return
            try:
                compressed = base64.b64decode(payload.get('data') or '')
                data = zlib.decompress(compressed)
            except Exception:
                self._FailCollabPatchSync('A host patch file could not be decoded.', disconnect=True)
                return

            expected_sha1 = str(expected.get('sha1') or '')
            actual_sha1 = self._TilesetBytesSha1(data)
            if expected_sha1 and actual_sha1 != expected_sha1:
                self._FailCollabPatchSync('A downloaded host patch file failed checksum verification.', disconnect=True)
                return

            staging_root = os.path.abspath(str(state.get('staging_root') or ''))
            dest_path = os.path.abspath(os.path.join(staging_root, *target_path.split('/')))
            if not dest_path.startswith(staging_root + os.sep):
                self._FailCollabPatchSync('A host patch file used an invalid path.', disconnect=True)
                return
            try:
                ...
                self._FailCollabPatchSync('Unable to write a downloaded host patch file.\n%s' % str(e), disconnect=True)
                return
        ...
        if msg_type == 'tileset_manifest':
            ...
            self._TouchCollabTilesetSyncProgress('sync.manifest_received')
            self._PrimeCollabTilesetSyncFromLocalFiles()
        ...
        if globals_.Area is None or globals_.Level is None:
            ...
            if msg_type in {'tileset_data', 'tileset_update'}:
                if msg_type == 'tileset_data' and isinstance(getattr(self, '_collabTilesetSyncState', None), dict):
                    try:
                        name, data, _slots = self._DecodeTilesetPayload(message.get('payload') or {})
                    except Exception:
                        name, data = None, None
                    if name and data:
        ...
        if self._CollabPatchSyncInProgress() and msg_type not in {'host_hello', 'peer_kicked', 'peer_banned', 'peer_rejected'}:
    ...
    def HandleRemoteSnapshot(self, level_data, area_num, sender):
        ...
        if self.IsCollabClientMode() and self.collabLastHash is None:
            ...
            if not host_level_name:
                self._CollabDebugLog('snapshot.defer.wait_host_level', sender=sender, area_num=area_num)
                self.collabPendingSnapshot = (level_data, area_num, sender)
                if hasattr(self, 'hoverLabel'):
                    self.hoverLabel.setText('Waiting for the host level information...')
                return
        if self._CollabPatchSyncInProgress():
            self._CollabDebugLog('snapshot.defer.patch_sync', sender=sender, area_num=area_num)
            self.collabPendingSnapshot = (level_data, area_num, sender)
            if hasattr(self, 'hoverLabel'):
                self.hoverLabel.setText('Waiting for the host patch download to finish...')
            return
        if self.collabApplyingRemote:
            return
        if self.IsLocalEditInProgress():
            self._CollabDebugLog('snapshot.defer.local_edit', sender=sender, area_num=area_num)
            self.collabPendingSnapshot = (level_data, area_num, sender)
            if hasattr(self, 'hoverLabel'):
        ...
        if self.IsCollabClientMode():
            ...
            if missing_tilesets:
                self._CollabDebugLog(
                    'snapshot.defer.missing_tilesets',
                    sender=sender,
                    area_num=area_num,
                    missing_tilesets=missing_tilesets,
                )
                self.collabPendingSnapshot = (level_data, area_num, sender)
                self._MaybeScheduleCollabTilesetSync(50)
                if hasattr(self, 'hoverLabel'):
        ...
        try:
            ...
            self._CollabDebugLog(
                ...
                digest=new_digest,
                initial_client_sync=initial_client_sync,
            ...
            self._CollabDebugLog('snapshot.apply.done', sender=sender, area_num=area_num, digest=new_digest)
        finally:
            if self.IsCollabClientMode():
                self._SetCollabMissingTilesetWarningsSuppressed(False)
            self.collabApplyingRemote = False
    ...
    def HandlePreferences(self):
        ...
        old_translation = setting('Translation')
        old_theme = setting('Theme')
        old_ui_style = setting('uiStyle')
        ...
        qp_value = dlg.generalTab.quickPaintShortcut.keySequence().toString(QtGui.QKeySequence.SequenceFormat.PortableText)
        qp_value = str(qp_value or '').split(',', 1)[0].strip() or qp_default
        setSetting('QuickPaintShortcut', qp_value)
        globals_.QuickPaintShortcut = qp_value

        ptr_default = str(getattr(globals_, 'CollabPointerShortcut', 'P') or 'P')
        ptr_value = dlg.generalTab.collabPointerShortcut.keySequence().toString(QtGui.QKeySequence.SequenceFormat.PortableText)
        ptr_value = str(ptr_value or '').split(',', 1)[0].strip() or ptr_default
        setSetting('CollabPointerShortcut', ptr_value)
        globals_.CollabPointerShortcut = ptr_value
        ...
        # Collaboration runtime logging setting
        collab_runtime_logs_enabled = dlg.generalTab.collabRuntimeLogs.isChecked()
        setSetting('CollabRuntimeLogsEnabled', collab_runtime_logs_enabled)
        if not collab_runtime_logs_enabled:
            try:
                os.remove(_get_collab_debug_log_path())
        ...
        new_theme = dlg.themesTab.themeBox.currentText()
        new_ui_style = dlg.themesTab.NonWinStyle.currentText()
        setSetting('Theme', new_theme)
        setSetting('uiStyle', new_ui_style)

        restart_needed = (
            str(old_translation) != str(name)
            or str(old_theme) != str(new_theme)
            or str(old_ui_style) != str(new_ui_style)
        )
    ...
    def _CollabClientHandleHostedLevelData(self, payload, sender):
        ...
        if self._CollabLoadHostedLevelBytes(level_name, level_bytes, area_num=area_num, host_path=host_path):
            ...
            try:
                if str(getattr(self, '_collabFocusCursorSid', '') or ''):
                    self._collabFocusCursorTimer.start(0)
            except Exception:
                pass
    ...
    def LoadLevelFromNetwork(self, levelData, areaNum):
        ...
        try:
            ...
            self._CollabDebugLog(
                'level.load.done',
                level_name=self._CollabCurrentLevelName(),
                area_num=int(getattr(getattr(globals_, 'Area', None), 'areanum', areaNum) or areaNum),
            )
            self.levelOverview.Reset()
            self.levelOverview.update()
            self._CollabRebuildIndexes()
            ...
            self.RestoreTransientUiState(ui_state)
            try:
                self._MaybeContinueDeferredCollabTilesetSync()
            except Exception:
                pass
        finally:
            self._CollabDebugLog(
                'level.load.finally',
                area_num=areaNum,
                current_level=self._CollabCurrentLevelName(),
                current_area=int(getattr(getattr(globals_, 'Area', None), 'areanum', 0) or 0),
            )
            globals_.OverrideSnapping = False
    ...
    def LoadLevel(self, name, isFullPath, areaNum):
        ...
        self._CollabDebugLog(
            ...
            area_num=areaNum,
            collab_mode=getattr(getattr(self, 'collabManager', None), 'mode', None),
        )
        new = name is None
        same = False
        ...
        if not new:
            if not restoring_autosave:
                checknames = []
                if isFullPath:
                    resolved_full_path = self._ResolveExistingLevelPath(name)
                    checknames = [resolved_full_path] if resolved_full_path else [name]
                else:
                    stage_path = str(globals_.gamedef.GetStageGamePath() or '')
                    base_candidate = os.path.join(stage_path, str(name or ''))
                    if re.search(r'(?i)\.arc(?:\.(?:lz|lh))?$', str(name or '')):
                        resolved_stage_path = self._ResolveExistingLevelPath(base_candidate)
                        checknames = [resolved_stage_path] if resolved_stage_path else [base_candidate]
                    else:
                        for ext in globals_.FileExtentions:
                            checknames.append(os.path.join(stage_path, str(name or '') + ext))
        ...
        try:
            ...
            self._PumpUiDuringAreaLoad('Loading area...')
            self._CollabDebugLog('level.load.phase', phase='loading_area')
            ...
            self._PumpUiDuringAreaLoad('Preparing area...')
            self._CollabDebugLog('level.load.phase', phase='preparing_area')
            ...
            self._PumpUiDuringAreaLoad('Finishing area...')
            self._CollabDebugLog('level.load.phase', phase='finishing_area')
    ...
    def LoadLevel_NSMBW(self, levelData, areaNum):
        ...
        """
        # Create the new level object
        self._CollabDebugLog('level.load_nsmw.begin', area_num=areaNum, level_bytes=len(levelData or b''))
        globals_.Level = Level_NSMBW()
        ...
        self.ResetPalette()
        self._CollabDebugLog('level.load_nsmw.done', area_num=areaNum)
    ...
    def _CanHandleCollabPointerKeyEvent(self, event):
    ...
    def _NormalizeSingleShortcutText(self, value):
    ...
    def _ShortcutTextToKeyAndModifiers(self, text):
        text = self._NormalizeSingleShortcutText(text)
        if not text:
            return (None, 0)
        ...
        try:
            ...
            key = None
        try:
            mods = int(kc.keyboardModifiers().value)
        except Exception:
            mods = 0
        return (key, mods)

    def _EventMatchesShortcut(self, event, key, mods):
        if key is None:
            return False
        if int(event.key()) != int(key):
            return False
        try:
            event_mods = int(event.modifiers().value)
        except Exception:
            event_mods = 0
        nonshift_mask = int((
        ...
        if (mods & nonshift_mask) == 0:
            return (event_mods & nonshift_mask) == 0
        return event_mods == mods

    def _GetCollabPointerShortcutKeyAndModifiers(self):
        default_text = str(getattr(globals_, 'CollabPointerShortcut', 'P') or 'P')
        return self._ShortcutTextToKeyAndModifiers(setting('CollabPointerShortcut', default_text))

    def _GetQuickPaintShortcutKeyAndModifiers(self):
        default_text = str(getattr(globals_, 'QuickPaintShortcut', 'O') or 'O')
        return self._ShortcutTextToKeyAndModifiers(setting('QuickPaintShortcut', default_text))

    def _IsCollabPointerShortcutEvent(self, event):
        key, mods = self._GetCollabPointerShortcutKeyAndModifiers()
        return self._EventMatchesShortcut(event, key, mods)

    def _IsQuickPaintShortcutEvent(self, event):
        key, mods = self._GetQuickPaintShortcutKeyAndModifiers()
        return self._EventMatchesShortcut(event, key, mods)

    def _HandleCollabPointerKeyPress(self, event):
        if not self._CanHandleCollabPointerKeyEvent(event):
            return False
        mode = self._NormalizeCollabCursorDisplayMode(getattr(self, 'collabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN))
        if mode == COLLAB_CURSOR_DISPLAY_NEVER:
            event.accept()
        ...
        self.collabPointerKeyHeld = True
        try:
            if hasattr(self, 'view') and self.view is not None:
                self.view.viewport().update()
        except Exception:
    ...
    def _HandleQuickPaintShortcut(self, event):
        if not self._CanHandleCollabPointerKeyEvent(event):
            return False
    ...
    def keyPressEvent(self, event):
        ...
        """
        if self._IsCollabPointerShortcutEvent(event) and self._HandleCollabPointerKeyPress(event):
            return

        if self._IsQuickPaintShortcutEvent(event) and self._HandleQuickPaintShortcut(event):
            return
    ...
    def keyReleaseEvent(self, event):
        if self._IsCollabPointerShortcutEvent(event) and self._CanHandleCollabPointerKeyEvent(event):
            mode = self._NormalizeCollabCursorDisplayMode(getattr(self, 'collabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN))
            if mode == COLLAB_CURSOR_DISPLAY_NEVER:
                event.accept()
                return
            if not event.isAutoRepeat() and getattr(self, 'collabPointerKeyHeld', False):
                self.collabPointerKeyHeld = False
                try:
                    if hasattr(self, 'view') and self.view is not None:
                        self.view.viewport().update()
...
def main():
    ...
    globals_.CollabCursorDisplayMode = str(setting('CollabCursorDisplayMode', getattr(globals_, 'CollabCursorDisplayMode', COLLAB_CURSOR_DISPLAY_ON_SCREEN)) or COLLAB_CURSOR_DISPLAY_ON_SCREEN)
    globals_.CollabPointerShortcut = str(setting('CollabPointerShortcut', getattr(globals_, 'CollabPointerShortcut', 'P')) or 'P')
    globals_.QuickPaintShortcut = str(setting('QuickPaintShortcut', getattr(globals_, 'QuickPaintShortcut', 'O')) or 'O')
    SLib.RealViewEnabled = globals_.RealViewEnabled
